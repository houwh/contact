        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="150">
            <Form-item label="产品分类:" prop="productClass">
                <Select v-model="formValidate.productClass" placeholder="请选择分类" style="width:40%">
                    <Option v-for="(item,index) in productClassData" :value="item.value" :label="item.label" :key="index"></Option>
                </Select>
            </Form-item>
            ...
            <Form-item label="唯一标识符:" prop="uniqueFlag" class="systemForm">
                <span>{{ formValidate.uniqueFlag }}</span>
            </Form-item>
        ...
        handleSubmit(name) {
            ...
            this.$refs[name].validate((valid) => {
                if (valid) {
                    ...
                    console.log(obj);
                    this.$store.dispatch('productEdit', obj).then(data => {
